...
for scene in scenes:
    ...
    if scene.end_time - scene.start_time >= 30:
        ...
        while current_time < scene.end_time:
            ...
            # Save the image analysis result to the collection
            print(f"Adding image analysis result to the collection")
        ...
        # Grader analysis
        grader_results = grader_agent.analyze(query="alcool", documents=collection_results.get("documents"))
        print(f"Grader results : {grader_results}")
        # Analyze the scene
        scene_analysis = scene_analysis_agent.analyze(context = grader_results)
